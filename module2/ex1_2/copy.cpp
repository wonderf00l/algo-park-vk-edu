...
class HashTable {
   ...
   public:
    ...
    bool operator[](const Key& key) const {
        ...
        size_t id = hasher(key, buckets_size), hash2 = !(id % 2) ? id + 1 : id, i = 1, step_counter = 0;
        while ((buckets[id].key != key ||  // !comp(buckets[id].key, Key())
                buckets[id].status != states::FILLED) &&
               step_counter <= buckets_quantity) {  // // check collision и учесть ситуацию, когда нужно добраться до
    ...
    bool add(const Key& key) {
        ...
        size_t id = hasher(key, buckets_size), hash2 = !(id % 2) ? id + 1 : id, i = 1;
        while (buckets[id].status == states::FILLED && i <= buckets_size) {  // check collision
    ...
    bool erase(const Key& key) {
        ...
        //     return false;
        // }
        size_t del_id = hasher(key, buckets_size), hash2 = !(del_id % 2) ? del_id + 1 : del_id, i = 1, step_counter = 0;
        while ((buckets[del_id].key != key ||  // !comp(buckets[del_id].key, Key())
                buckets[del_id].status == states::DELETED) &&
               step_counter <= buckets_quantity) {
            del_id = prob_step(del_id, hash2, i++);
            ++step_counter;
        }
        if (step_counter >= buckets_quantity) {
            return false;
        }
        buckets[del_id].status = states::DELETED;
    ...
    void rehash() {
        ...
        for (Bucket& bucket : buckets) {
            if (bucket.status == states::FILLED) {
                size_t new_id = hasher(bucket.key, buckets_size), hash2 = !(new_id % 2) ? new_id + 1 : new_id, i = 1;
                while (new_buckets[new_id].status == states::FILLED && i <= buckets_quantity) {
                    new_id = prob_step(new_id, hash2, i++);
                }
                new_buckets[new_id] = std::move(bucket);
                if (i >= buckets_quantity) {
                    return;
                }
            }
        }
