...
class ProbHasher<std::string> {
   public:
    size_t operator()(const std::string& str, size_t m) const { return str_hash(str, m); }
...
class HashTable {
   ...
   public:
    ...
    bool operator[](const Key& key) const {
        ...
        size_t hash1 = hasher(key, buckets_size), hash2 = prob_hasher(key, buckets_size);
    ...
    bool add(const Key& key) {
        ...
        size_t hash1 = hasher(key, buckets_size), hash2 = prob_hasher(key, buckets_size), first_deleted = 0, i = 1;
        bool got_deleted = 0;

        for (; buckets[hash1].status != states::EMPTY && i <= buckets_size;
             hash1 = prob_step(hash1, hash2, i++)) {
            if (comp(buckets[hash1].key, key)) {  // buckets[hash1].status == states::FILLED -- предусмотрено
                return false;
            }

            if (buckets[hash1].status == states::DELETED &&
                !got_deleted) {  // при принудительном выходе из цикла пробирования пишем ключ в бакет deleted
                first_deleted = hash1;
                got_deleted = 1;
            }
        }
        ...
        if (i > buckets_size) {
            assert(got_deleted); // если принудительно вышли из цикла пробирования, то должны былии хоть раз наткнуться на ячейку deleted
        }
    ...
    void rehash() {
        ...
        for (Bucket& bucket : buckets) {
            if (bucket.status == states::FILLED) {
                size_t hash1 = hasher(bucket.key, buckets_size), hash2 = prob_hasher(bucket.key, buckets_size);
