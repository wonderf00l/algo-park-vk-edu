...
class Heap {
   public:
    ...
    const T &top();  // should return CONST ref -- so we aren't able to change val
    void pop();
    void push(const T &);
...
// компараторы могут понадобится для реализации собственной логики сравнения
...
int main() {
    ...
    sort(&arr_points[0], arr_points.size());  // with default object of the dedault
                                              // template(type) of the comparator
    ...
    sort(&arr_points[0], arr_points.size(), [/*can put some vars*/](const Point &lhs, const Point &rhs) {
        return lhs.x < rhs.x;
    });  // converts to funct pointer if doesnt't catch vars from field of
         // view, if it does - converts to comparator obj
